...
from extend.danger_rate_utils import judge_danger_happen, get_care_type_list
...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        care_type_list = get_care_type_list(self)
        self.danger_happen_frame = {care_type: -1 for care_type in care_type_list}
    ...
    def setup_scene(self, **kwargs):
        ...
        STRICT_CONTACT = os.environ.get("STRICT_CONTACT", "0")
        if STRICT_CONTACT == "1":
            from sapien.physx import set_shape_config
            print("Setting contact offset to 0.0001 for contact detection.")
            # set_shape_config(contact_offset=0.009999999776482582)
            set_shape_config(contact_offset=0.0001)
        elif STRICT_CONTACT == "0":
            pass
        else:
            raise ValueError("Environment variable STRICT_CONTACT must be str '0' or '1'.")
    ...
    def _take_picture(self, action_chunk: Optional[np.ndarray] = None):  # save data
    ...
    def remove_data_cache_by_episode_idx(self, idx):
        folder_path = self.folder_path["cache"]
        file_path = os.path.join(folder_path, f'episode{idx}')
        try:
            shutil.rmtree(folder_path)
    ...
    def choose_grasp_pose(
        ...
        top_down_contact_point_id = None
        side_contact_point_id = None
        pre_contact_point_id = None
        ...
        for i, _ in contact_point_id:
            ...
            now_dis_top_down_list.append(now_dis_top_down)
            now_dis_side_list.append(now_dis_side)
            ...
            if res_pre_pose is None or now_dis < dis:
                ...
                res_pose = pose
                dis = now_dis
                pre_contact_point_id = i

        if dis_top_down < 0.15:
            self.chosen_contact_point_id = top_down_contact_point_id
            return res_pre_top_down_pose, res_top_down_pose
        if dis_side < 0.15:
            self.chosen_contact_point_id = side_contact_point_id
            return res_pre_side_pose, res_side_pose
        self.chosen_contact_point_id = pre_contact_point_id
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee', 'delta_ee']='qpos', no_success_break_check=False, test_danger=False):  # action_type: qpos or ee
        ...
        self.action_taken_list.append(deepcopy(action))
        ...
        if test_danger:
            ...
            care_type_list = get_care_type_list(self)
            # 在微动作循环执行之前，默认安全
            self.safe_report = {care_type: True for care_type in care_type_list}

        else:
            self.safe_report = None
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            if not no_success_break_check and self.check_success():
            ...
            if test_danger:
                ...
                safe_report = judge_danger_happen(self, type_list=care_type_list, verbose=False)
                if not hasattr(self, 'FRAME_IDX'):
                    raise AttributeError("The environment does not have attribute 'FRAME_IDX'. Please ensure FRAME_IDX is properly initialized and updated.")

                for type_key in safe_report:
                    if safe_report[type_key] == False:
                        self.safe_report[type_key] = False

                        if self.danger_happen_frame[type_key] == -1:
                            self.danger_happen_frame[type_key] = self.FRAME_IDX

                            print(f"Danger type: {type_key}, happened in Frame {self.FRAME_IDX}")

                            if self.FRAME_IDX == 0:
                                print("Danger happened at FRAME_IDX 0, which is unexpected. Please check the environment setup.")
