...
def main(task_name=None, task_config=None):
    ...
    args["save_env_info"] = args.get("save_env_info", False)
    run(task, args)
...
def run(TASK_ENV, args):
    ...
    if args["save_env_info"]:
        ...
        # reset _take_picture to save env info
        def take_picture_with_env_info(self):
            ...
            env_dic = env_info_save_fn(self, return_tensors="np")
            pkl_dic['env'] = env_dic
            save_pkl(self.folder_path["cache"] + f"{self.FRAME_IDX}.pkl", pkl_dic)  # use cache
            self.FRAME_IDX += 1
        
        # replace _take_picture with take_picture_with_env_info
        TASK_ENV._take_picture = types.MethodType(take_picture_with_env_info, TASK_ENV)   
