...
def load_hdf5(dataset_path):
    ...
    with h5py.File(dataset_path, "r") as root:
        ...
        env_dict = {}
        if "env" in root:
            for key in root["/env"].keys():
                env_dict[key] = root[f"/env/{key}"][()]
...
def main():
    ...
    if args.timestamp is not None:
        load_dir = os.path.join(load_dir, args.timestamp)
        ...
        for j in range(0, left_gripper_all.shape[0]):
            ...
            if j != left_gripper_all.shape[0] - 1:
                ...
                for key in env_dict_all.keys():
                    env_arrays[key].append(env_dict_all[key][j])
    ...
    for key in env_arrays.keys():
        env_arrays[key] = np.array(env_arrays[key])
